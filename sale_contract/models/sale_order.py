...
class SaleOrder(models.Model):
    ...
    def _compute_subscription(self):
        for order in self:
            order.subscription_id = self.env['sale.subscription'].search([('analytic_account_id', '=', order.analytic_account_id.id)], limit=1)
    ...
    def _search_subscription(self, operator, value):
        if value == False:  # looking for set/unset values
            if operator == '=':  # aa without subscription or no aa at all
                ...
                search_domain = ['|', ('analytic_account_id', 'in', aa_accounts.ids), ('analytic_account_id', '=', False)]
            if operator == '!=':  # aa with subscriptions
                search_domain = [('analytic_account_id.subscription_ids', '!=', False)]
        else:  # text/id search
            try:
                value = isinstance(value, (bytes, str)) and int(value) or value  # id search
                ...
                search_domain = [('analytic_account_id', 'in', aa_ids)]
            except ValueError:
    ...
    def _prepare_invoice(self):
        ...
        if self.analytic_account_id and self.subscription_management == 'renew':
            subscr = self.env['sale.subscription'].search([('analytic_account_id', '=', self.analytic_account_id.id)], limit=1)
